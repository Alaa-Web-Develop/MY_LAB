...
@section('content')
    ...
    <div class="col-12 card">
        ...
        @else
        <table class="table table-striped" style="background-color: rgba(23, 29, 22, 0.698);color:white;">
            <thead style="background-color:rgba(23, 29, 22, 0.698);color:whitesmoke;"">
                <tr>
            ...
            <tbody>
                @foreach ($actions as $action)
                    <tr>
                        <td>{{ $action->action }}</td>
                        <td>{{ $action->type }}</td>
                        <td>{{ date('d/m/Y H:i A', strtotime($action->action_date)) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
